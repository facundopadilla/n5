...
class TestPolioceOfficerIntegration:
    URL = "/api/v1/police_officers"

    async def test_create_successful(
            self,
            async_client: httpx.AsyncClient,
            mock_session: AsyncSession,
            fake_police_officer_dict: dict
    ):
        # region -- Setup --
        fake_police_officer_dict["password"] = "test"
        # endregion
        # region -- Assert --
        rsps = await async_client.post(url=self.URL, json=fake_police_officer_dict)
    ...
    async def test_police_officer_filter(
            self,
            async_client: httpx.AsyncClient,
            mock_session: AsyncSession,
            fake_police_officer
        ...
        powerset = powerset_dict(fake_police_officer.model_dump(exclude=exclude, exclude_none=True))
        # region -- Assert --
        for query_params in powerset:
            rsps = await async_client.get(
                url=self.URL + "/filter", params=query_params
            )
            expected_response = fake_police_officer.model_dump(exclude=exclude, exclude_none=True)
            data = rsps.json()
            items = {
                k: v
